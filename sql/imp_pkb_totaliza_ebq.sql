PROMPT CREATE OR REPLACE PACKAGE BODY imp_pkg_totaliza_ebq
CREATE OR REPLACE PACKAGE BODY imp_pkg_totaliza_ebq AS
  -- $Date:                                                         $
  -- $Revision:            $
...
--      vn_pesobrt_ebq := vn_pesobrt_ebq+vn_soma_pesobrt_rem;
  ...
  BEGIN
    ...
    IF ( Nvl(cmx_pkg_tabelas.auxiliar(vn_tp_embarque_id,1),'XX') <> 'NAC_DE_TERC' ) THEN
      ...
      IF( (   (  pn_tp_declaracao < 13 OR vc_global_nac_sem_admissao  = 'S' )
          AND ( Nvl(vc_embarque_nac,'XX') NOT IN ('NAC', 'NAC_TERC')        )
          )
...
--    cmx_prc_teste_Execucao ('vn_ultima_adi_ad['||vn_ultima_adi_ad||'] ptb_ad_adi.count['||ptb_ad_adi.count||'] ptb_ad_adi.last['||ptb_ad_adi.last||']', 9988);
  ...
  BEGIN
    ...
    AND (vn_fr_invoice <> vn_fr_conhecimento)
    THEN
      prc_gera_log_erros ('A soma dos fretes destacados nas invoices é diferente do valor do frete informado no conhecimento da DA. [Frete Invoice(s):  USD '||rtrim(ltrim(to_char(vn_fr_invoice,'999G999G999G990D00')))||'][Frete Conhecimento da Admissão: USD '||rtrim(ltrim(to_char(vn_fr_conhecimento,'999G999G999G990D00')))||']','E', 'E');
    END IF;
  EXCEPTION
    WHEN others THEN
...
--          END IF;
    ...
    PROCEDURE imp_prc_calcula_impostos_adi (pn_embarque_id NUMBER, pn_tp_declaracao NUMBER, pn_empresa_id NUMBER, pc_dsi VARCHAR2) IS
      ...
      CURSOR cur_quantidade_antidumping (pn_declaracao_adi_id NUMBER) IS
        SELECT sum (decode (idl.tipo_unidade_medida_antid, 'P', pesoliq_tot, qtde))
          FROM imp_declaracoes_lin idl
         WHERE idl.declaracao_adi_id = pn_declaracao_adi_id;
...
--**fim codigo adicionado
          ...
          IF (cr_adi.vr_aliq_esp_antdump IS NOT null) THEN
            ...
            FETCH cur_quantidade_antidumping INTO tb_dcl_adi(adi).qt_um_aliq_esp_antdump;
        ...
        FOR x IN cur_tmp_ficha LOOP
          ...
          ELSIF x.tipo_ficha_cambio <> 'P' THEN
            ...
            ELSE
              ...
              IF (length (x.numero_contrato) > 11) THEN
