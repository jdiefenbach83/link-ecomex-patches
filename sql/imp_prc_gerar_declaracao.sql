REM +==========================================================================================+
REM |                                                                                          |
REM | Descricao: O objetivo desta procedure é, a partir de um numero de embarque ou invoice,   |
REM |            gerar a declaracao de importacao - cabecalho e linhas. As adicoes sao geradas |
REM |            atraves de triggers.                                                          |
REM |            Se o usuario cadastrou varias invoices, fez a vinculacao do Embarque e esta   |
REM |            pronto para criar a Declaracao, ele devera entrar no browser de Declaracao e  |
REM |            utilizar a funcao 'INCLUIR'. Esta procedure entao é chamada, e recebe, entre  |
REM |            outros parametros o Embarque_id. A partir dele, iremos incluir na             |
REM |            declaracao todas as Invoices vinculadas neste Embarque, que tenham ou nao LI. |
REM |            No entanto, se depois deste processo, o usuario esqueceu-se de cadastrar uma  |
REM |            Invoice e de vincula-la no Embarque, ele pode fazer isso, ou seja, cadastra a |
REM |            nova Invoice, vincula-a no Embarque, entra no browser de Declaracao, edita    |
REM |            a Declaracao ja criada e utiliza a funcao "ADICIONAR INVOICE". Neste momento  |
REM |            esta procedure é chamada e recebe como parametro principal o campo Invoice_id,|
REM |            e inclui na Declaracao ja existente a invoice e suas linhas.                  |
REM |            Esta procedure sera utilizada, principalmente, pela tela browser de           |
REM |            Declaracao, no botao "INCLUIR" ou na funcao "ADICIONAR INVOICE".              |
REM |            Os parametros devem estar preenchidos nas seguintes situacoes:                |
REM |                                                                                          |
REM |             PARAMETRO                      BOTAO NOVO    ADICIONAR INVOICE               |
REM |            +==============================+=============+=================+              |
REM |            |pn_empresa_id                 | preenchido  | preenchido      |              |
REM |            |pn_embarque_id                | preenchido  | preenchido      |              |
REM |            |pc_flag_dsi                   | preenchido  | preenchido      |              |
REM |            |pn_tp_declaracao_id           | preenchido  | preenchido      |              |
REM |            |pc_referencia                 | preenchido  | null            |              |
REM |            |pn_user_id                    | preenchido  | preenchido      |              |
REM |            |pn_declaracao_id *            | IN OUT      | IN OUT          |              |
REM |            |pn_invoice_id                 | null        | preenchido      |              |
REM |            |pc_gera_li_drawback           | 'S' ou 'N'  | 'S' ou 'N'      |              |
REM |            |pc_quebra_qtde_drawback       | 'S' ou 'N'  | 'S' ou 'N'      |              |
REM |            |pn_evento_id                  | OUT         | OUT             |              |
REM |            |pb_li_gerada                  | OUT         | OUT             |              |
REM |            |pn_da_id                      | DI='14'     | DI='14'         |              |
REM |            |pn_fins_de_exportação         | 'S' ou 'N'  | null            |              |
REM |            |pc_fins_cambiais              | 'S' ou 'N'  | null            |              |
REM |            +==============================+=============+=================+              |
REM |                                                                                          |
REM |            * O parametro pn_declaracao_id é IN OUT e retornara no caso de uma Nova       |
REM |            declaracao, o ID desta declaracao, e no caso de Adicao de Invoice,            |
REM |            ele DEVE receber o ID da Declaracao que ira receber a invoice e retornaremos  |
REM |                                                                                          |
REM |            O parametro pc_gera_li_drawback indica se será gerada LI para os itens que    |
REM |            tenham saldo para drawback, se o parâmetro for 'S' será verificado se tem     |
REM |            saldo(dentro das condições de validade do rd).                                |
REM |            O parametro pc_quebra_qtde_drawback se for 'S', indicará que a quantidade     |
REM |            da linha da invoice será quebrada de acordo com o saldo do rd, ou seja,       |
REM |            será gerada uma linha de LI com o saldo do RD e o restante será gerado        |
REM |            em uma linha de declaração. Se o parâmetro for 'N', será gerada somente a     |
REM |            linha de declaração.                                                          |
REM |                                                                                          |
REM |            O parametro pn_evento_id serve para indicar que aconteceu um erro na geração  |
REM |            automática de LI de drawback. Se este parametro não for nulo é pq gerou erro. |
REM |                                                                                          |
REM |            O parametro pb_li_gerada foi adicionada para mostrar para o                   |
REM |            usuário que a LI de drawback foi gerada com sucesso, quando isto realmente    |
REM |            acontecer, ou seja, quando não houver log de erro ou de alerta.               |
REM |                                                                                          |
REM +==========================================================================================+
CREATE OR REPLACE PROCEDURE imp_prc_gerar_declaracao (
  ...
  */
  CURSOR cur_declaracao_lin_com_li (pn_inv_id imp_invoices.invoice_id%TYPE) IS
    SELECT il.embarque_id
         ...
         , NVL((SELECT aliq_adval_igual_red
                 FROM imp_excecoes_fiscais ex
                WHERE ex.excecao_id= ill.excecao_ii_id),'N') aliq_adval_igual_red
      ...
      */
      ...
      IF (pc_flag_dsi = 'S') THEN
        ...
        */
        ...
        OPEN cur_exfiscal_ii(vn_excecao_ii_id);
        FETCH cur_exfiscal_ii INTO vc_aliq_adval_igual_red;
        CLOSE cur_exfiscal_ii;

        vr_exfiscal_antd := NULL;
        IF vn_excecao_antid_id IS NOT NULL THEN
          OPEN cur_exfiscal_antd(vn_excecao_antid_id);
         ...
         IF (pc_flag_dsi = 'S') THEN
          ...
          END IF;
           ...
           vn_adi_id := imp_fnc_gera_adicao_dcl (
                          ...
                          , vc_beneficio_suspensao               /* pc_beneficio_suspensao         */
                          , vn_perc_beneficio_suspensao          /* pn_perc_beneficio_suspensao    */
                          , cdll.aliq_adval_igual_red          /* pc_aliq_adval_igual_red       */
                          , vr_exfiscal_antd.ato_legal_id      /* pn_ato_legal_antd_id          */
                          , vr_exfiscal_antd.orgao_emissor_id  /* pn_orgao_emissor_antd_id      */
                          , vr_exfiscal_antd.nr_ato_legal      /* pn_nr_ato_legal_antd          */
                          , vr_exfiscal_antd.ano_ato_legal     /* pn_ano_ato_legal_antd         */
