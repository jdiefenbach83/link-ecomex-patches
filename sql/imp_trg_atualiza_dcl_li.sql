REM +==========================================================================================+
REM | Descricao: Gerar linhas de Declaracao ou apagar linhas de Declaracao, a partir da        |
REM |            atualizacao do registro de uma Licenca.                                       |
REM |            Quando o usuario informar o numero de registro de uma Licenca de Importacao   |
REM |            devemos criar as linhas da Declaracao baseadas nas linhas da Licenca.         |
REM |            Para este processo ocorrer, a Declaracao ja tem que estar criada. Se ainda    |
REM |            nao foi criada, nao precisamos incluir suas linhas aqui, pois posteriormente, |
REM |            quando o usuario criar a Declaracao, as linhas da Licenca serao lidas e       |
REM |            inseridas como linhas de Declaracao automaticamente.                          |
REM |            Da mesma forma, se o usuario apagar o numero do Registro da Licenca, devemos  |
REM |            apagar todas as linhas da Declaracao, referentes às linhas da Licenca em      |
REM |            evidencia.                                                                    |
REM +==========================================================================================+

CREATE OR REPLACE TRIGGER imp_trg_atualiza_dcl_li
AFTER UPDATE
OF NR_REGISTRO
ON imp_licencas
FOR EACH ROW
...
DECLARE
  ...
  CURSOR cur_declaracao_id_nac IS
  ...
  CURSOR cur_declaracao_id IS
    SELECT declaracao_id
     ...
     WHERE embarque_id = :NEW.embarque_id;
  ...
  CURSOR cur_invoice (pn_invoice_id NUMBER) IS
    SELECT moeda_id
      FROM imp_invoices
    ...
    SELECT ill.invoice_id
         ...
         , (SELECT ief.ins_estadual_id
             ...
             WHERE cie.ins_estadual_id = ief.ins_estadual_id
               AND cie.empresa_id      = ii.empresa_id
  ...
  CURSOR cur_lin_di_retif(pn_licenca_id NUMBER) IS
    SELECT NULL
      FROM imp_licencas_lin
     WHERE licenca_id = pn_licenca_id
       AND Nvl(gerado_di_retif, 'N') = 'S';

  CURSOR cur_declaracao_adi IS --chl
    SELECT declaracao_adi_id
      FROM imp_licencas_lin
     WHERE licenca_id = :old.licenca_id
       AND declaracao_adi_id IS NOT NULL;

  CURSOR cur_linhas_di IS --chl
    SELECT Count(1)
      FROM imp_licencas_lin
     WHERE licenca_id = :old.licenca_id;
  ...
  CURSOR cur_icms IS
    SELECT Nvl(tratar_icms_deso_emiss_nfe, 'N') AS tratar_icms_deso_emiss_nfe
  ...
  vn_nr_declaracao              imp_declaracoes.nr_declaracao%TYPE;
...
BEGIN
  ...
  /*
    Verifica se a LI foi gerada apartir de uma DI retificada.
    Se foi gerada apartir de uma DI retificada, não inseri ou exclui a linha da DI
  */
  OPEN  cur_lin_di_retif(:new.licenca_id);
  FETCH cur_lin_di_retif INTO vn_temp;
  vb_achou := cur_lin_di_retif%FOUND;
  CLOSE cur_lin_di_retif;

  IF NOT(vb_achou) THEN

    /* Devo saber qual o ID da declaracao que ira receber as linhas de LI, para tanto,
      acesso a declaracao_id atraves do numero do embarque desta LI.
    */
    vc_step_error := 'cur_declaracao_id';
    IF (:new.embarque_id IS NOT null) THEN
    ...
    /* So posso criar as linhas da declaracao, se o usuario ja criou o cabecalho da declaracao.
      ...
      Da mesma forma, so preciso tratar a alteracao/delecao do registro para null se ja tivermos
      linhas de declaracao.
    */
    IF (vn_declaracao_id is not null) THEN
      IF (:new.nr_registro IS null AND :old.nr_registro IS NOT null ) THEN
        /* Se o numero de Licenca foi removido, devemos deletar as linhas de declaracao
            correspondentes às linhas desta Licenca, no entanto, devemos backupear alguns valores
            da declaracao, para que, quando o usuario retornar com o numero da Licenca podermos
            inserir novamente as linhas de declaracao com os valores originais.
            No entanto, so posso permitir esta alteracao, se a declaracao envolvida nao tiver
            sido registrada ainda, pois, se ja foi registrada o usuario deverar remover
            o numero da declaracao antes de alterar qualquer licenca envolvida no processo.
        */
        IF (vn_nr_declaracao is null) Then
          vc_step_error := 'UPDATE imp_licencas_lin 1';

          /* Criado loop para os casos em que a linha da DI é quebrada por DA */
          FOR cur_bkpdcl IN (SELECT idl.declaracao_lin_id
                                FROM imp_declaracoes_lin  idl
                                  , imp_licencas_lin     ill
                              WHERE idl.licenca_lin_id = ill.licenca_lin_id
                                AND ill.licenca_id     = :old.licenca_id
                              )

          LOOP
            UPDATE imp_licencas_lin
              SET (  naladi_ncca_id_bkpdcl
                    , vida_util_bkpdcl
                    )
                  = (SELECT idl.naladi_ncca_id
                          , idl.vida_util
                      FROM imp_declaracoes_lin idl
                      WHERE idl.declaracao_lin_id = cur_bkpdcl.declaracao_lin_id
                    )
                WHERE licenca_id = :old.licenca_id;
          ...
          vc_step_error := 'DELETE imp_declaracoes_lin 1';
          FOR d IN ( SELECT declaracao_lin_id
                       FROM imp_declaracoes_lin
                       WHERE licenca_id = :old.licenca_id ) LOOP

            DELETE
              FROM imp_adm_temp_entradas
             WHERE declaracao_lin_id = d.declaracao_lin_id;

            DELETE
              FROM imp_entrep_ad_entradas
             WHERE declaracao_lin_id = d.declaracao_lin_id;

            UPDATE imp_numerarios_itens
               SET declaracao_lin_id = NULL
             WHERE declaracao_lin_id = d.declaracao_lin_id;
          ...
          vc_step_error := null;
        ELSE
          raise_application_error (-20000, 'IMP_TRG_ATUALIZA_DCL_LI - Alterar o n.o do Registro da LI, neste momento não é permitido. A Declaração (n.o ' || vn_nr_declaracao || ') envolvida neste processo, já foi registrada.');
        END IF;

      ELSIF (:old.nr_registro IS null AND :new.nr_registro IS NOT null ) THEN
        ...
        */
        ...
        vc_step_error := 'Atualizando URFE e URFD da declaração.';
        UPDATE imp_declaracoes
          SET urfe_id = :new.urfe_id
            , urfd_id = :new.urfd_id
        WHERE declaracao_id = vn_declaracao_id;
        vc_step_error := null;
        ...
        */
        FOR cll IN cur_linhas_licencas LOOP
          ...
          OPEN cur_invoice (cll.invoice_id);
          FETCH cur_invoice INTO vn_moeda_id;
          CLOSE cur_invoice;
          ...
          */
          vn_adi_id := Null;
          IF (:old.nat_operacao_id IS NOT null) THEN
            ...
            */
            ...
            vc_step_error := 'imp_fnc_gera_adicao_dcl';
              ...
              vc_step_error := null;
          END IF; /* (:old.nat_operacao_id is not null) */
          ...
          IF (vc_tp_declaracao IN ('14','15') AND Nvl(vc_nac_sem_vinc_admissao,'N') = 'N') THEN
            imp_pkg_nacionalizacao.gerar (
                ...
                , cll.rd_id
                , vn_moeda_id  /* Para gerar a LI de Drawback precisamos da moeda id */
                , vn_linha_num
                , vn_evento_id
          ...
          ELSE
            OPEN  cur_icms;
            FETCH cur_icms INTO vc_tratar_icms_deso_emiss_nfe;
            CLOSE cur_icms;

            IF(vc_tratar_icms_deso_emiss_nfe = 'S')THEN
              vn_icms_motivo_desoneracao_id := NULL;
              OPEN cur_excecao_motivo_desoneracao(cll.excecao_icms_id);
            ...
            , icms_motivo_desoneracao_id      /* 118 */
            )
            VALUES (
              decode (:old.nat_operacao_id, null, null, vn_adi_id)                            /* 00 */
            ...
            , vn_icms_motivo_desoneracao_id                                                   /* 118 */
            );
            ...
            FETCH cur_declaracao_adi INTO vn_adi_id;
            vb_achou := cur_declaracao_adi%FOUND;
            CLOSE cur_declaracao_adi;

            IF vb_achou THEN
              OPEN cur_linhas_di;
              FETCH cur_linhas_di INTO vn_temp;
              CLOSE cur_linhas_di;

              UPDATE imp_declaracoes_adi
                 SET qtde_linhas = vn_temp
               WHERE declaracao_adi_id = vn_adi_id;
            END IF;
          END IF;
      ...
      END IF; /* (:NEW.nr_registro IS null) - ELSIF (:old.nr_registro IS null) */
    END IF; /* (vn_declaracao_id is not null) */

  END IF; --vb_achou DI retificada

EXCEPTION
  WHEN others THEN
    IF (vn_evento_id IS NOT NULL) THEN
      raise_application_error (-20000, 'IMP_TRG_ATUALIZA_DCL_LI [' || sqlerrm || ']. Erro na criação da linha da Declaração a partir de uma Licença. [' || vc_step_error || '] - Mais detalhes no evento do sistema '||vn_evento_id);
    ELSE
      raise_application_error (-20000, 'IMP_TRG_ATUALIZA_DCL_LI [' || sqlerrm || ']. Erro na criação da linha da Declaração a partir de uma Licença. [' || vc_step_error || ']');
    END IF;
END imp_trg_atualiza_dcl_li;
/