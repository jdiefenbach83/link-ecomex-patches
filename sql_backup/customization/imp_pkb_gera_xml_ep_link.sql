CREATE OR REPLACE PACKAGE imp_pkg_gera_xml_ep_link
AS
  ...
  PROCEDURE imp_prc_gera_dsi_lines (  pc_moeda_frete              VARCHAR2
...
END imp_pkg_gera_xml_ep_link;

/

PROMPT CREATE OR REPLACE PACKAGE BODY imp_pkg_gera_xml_ep_link
CREATE OR REPLACE PACKAGE BODY imp_pkg_gera_xml_ep_link AS
  ...
  IS
    ...
    SELECT id.tp_declaracao_id
         ...
         , imp_fnc_busca_dt_cronog(id.embarque_id, 'EMBARQUE', 'R') data_embarque
           -- CUSTOMIZACAO
         , Nvl(imp_fnc_busca_dt_cronog(id.embarque_id, 'TRANSBORDO_NAC' , 'R'), imp_fnc_busca_dt_cronog(id.embarque_id, 'CHEGADA' , 'R')) data_chegada
           -- FIM CUSTOMIZACAO
...
BEGIN
  ...
  FOR i IN cur_declaracoes_adi LOOP
    ...
    --NSI-1199 IF pc_dcl_retif IS null THEN
      ...
      IF (i.ex_ipi_id IS NOT null) OR ( i.ato_legal_exipi_id IS NOT null ) THEN
      ...
      IF (i.ato_legal_exprac_id IS NOT null) THEN
