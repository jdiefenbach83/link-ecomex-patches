        ...
        FOR ivc IN 1..vn_global_ivc_lin LOOP
          IF (tb_ivc_lin(ivc).tp_linha_codigo <> 'E') THEN  -- não considera peso bruto nas linhas de embalagem retornável
            IF (ivc = vn_global_ivc_lin) THEN
              tb_ivc_lin(ivc).pesobrt_tot := vn_pesobrt_ebq - vn_soma_rateio_pesobrt;
            ELSE
    ...
    AND (  vr_mod_despacho.mde_despacho           NOT IN ('2' , '4', '6')
      ...
      ELSE
        ...
        FOR ivcR IN 1..vn_global_ivc_lin LOOP
          ...
          IF NOT(vb_ratear_td_itens) THEN

            vn_qtde_itens := 0;
            OPEN cur_qtde_itens(tb_ivc_lin(ivcR).invoice_lin_id);
            FETCH cur_qtde_itens INTO vn_qtde_itens;
            CLOSE cur_qtde_itens;

            OPEN  cur_qtde_adi(tb_ivc_lin(ivcR).invoice_lin_id);
...
--            vn_vlrt_ii_ipi := vn_vlrt_ii_ipi / vn_taxa_usd;
        ...
        FOR dcl IN 1..vn_global_dcl_lin LOOP
          ...
          IF NOT(vb_ratear_td_itens) THEN
            OPEN  cur_ratear_di_lin(tb_dcl_lin(dcl).declaracao_lin_id);
            FETCH cur_ratear_di_lin INTO vn_dummy;
            vb_ratear := cur_ratear_di_lin%FOUND;
            CLOSE cur_ratear_di_lin;
          ELSE
            vb_ratear := TRUE;
          END IF;
  ...
  PROCEDURE imp_prc_calcula_impostos ( pn_embarque_id    NUMBER
    ...
    PROCEDURE imp_carrega_adicao_dcl ( pn_declaracao_adi_id     NUMBER
      ...
      BEGIN
        ...
        ELSE
          IF (nvl (vn_um_siscomex,'XX') = '21') AND (vn_fator_conversao <> 0) THEN --Jefferson
            RETURN (pn_pesoliq_tot * vn_fator_conversao);
          ELSIF (vn_fator_conversao <> 0) THEN
...
--         , dcl_totalizada                       = vc_global_dcl_totalizada
         ...
         , vrt_ii_proporcional                  = vn_global_vrt_ii_prop_mn
         , vrt_ipi_proporcional                 = vn_global_vrt_ipi_prop_mn
         , vrt_pis_proporcional                 = vn_global_vrt_pis_prop_mn
         , vrt_cofins_proporcional              = vn_global_vrt_cofins_prop_mn
    ...
    BEGIN -- fnc_calcula_qtde_um_estat_li
      ...
      ELSE
        ...
        ELSIF (nvl (vc_um_siscomex,'XX') = '10') THEN
          return (pn_pesoliq);
        ELSE
          IF (nvl (vc_um_siscomex,'XX') = '21') AND (vb_achou_fator) THEN --Jefferson
            return (pn_pesoliq * vn_fator_conversao);
          ELSIF (vb_achou_fator) THEN
            return (pn_qtde * vn_fator_conversao);
          ELSE
            return (pn_qtde);
          END IF; -- IF (vb_achou_fator) THEN
